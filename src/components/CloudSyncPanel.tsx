...
import { APP_SESSION_TOKEN_KEY, applySnapshot, createNativeAuthReturnTo, jsonFetch, readSnapshot, type Snapshot } from "@/lib/cloudSync";
...
type CloudState = { user: User | null; loading: boolean; busy: boolean; message: string; calendarEnabled: boolean };
...
export default function CloudSyncPanel() {
  ...
  const syncCalendar = async (enabled = state.calendarEnabled) => {
    ...
    try {
      ...
      setState(s => ({
        ...
        message: res.enabled ? `Calendar 已同步 ${res.synced} 条，跳过 ${res.skipped} 条` : "Calendar 同步已关闭",
      }));
    } catch (err) {
      ...
      if (needsCalendarConsent && enabled) {
        const returnTo = isNativeApp() ? createNativeAuthReturnTo("calendar") : "";
        void openInAppBrowser(`/api/auth/google/calendar?returnTo=${returnTo}`);
      }
    }
  };
  ...
  if (state.loading) return <div style={{ padding: 18, color: "var(--text-sec)", fontSize: 13 }}>正在读取账户状态</div>;
    ...
    return (
      <div style={{ padding: 18, display: "grid", gap: 12 }}>
        <div>
          ...
          <div style={{ marginTop: 4, fontSize: 12, color: "var(--text-sec)", lineHeight: 1.55 }}>未登录时只保存在本机浏览器；登录后自动备份任务、专注记录和设置。</div>
        </div>
        <button
          type="button"
          onClick={() => {
            const returnTo = isNativeApp() ? createNativeAuthReturnTo("signin") : "";
            void openInAppBrowser(`/api/auth/google?returnTo=${returnTo}`);
          }}
          className="pressable"
          style={{ textAlign: "center", borderRadius: 18, padding: "12px 14px", background: "var(--text)", color: "var(--bg)", fontSize: 14, fontWeight: 850, textDecoration: "none" }}
        >
          连接 Google
        </button>
    ...
    <div style={{ padding: 18, display: "grid", gap: 12 }}>
      ...
      <div style={{ borderRadius: 16, padding: "12px 14px", background: "var(--control-bg)", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
        <div style={{ minWidth: 0 }}>
          ...
          <div style={{ marginTop: 2, fontSize: 11, color: "var(--text-sec)", lineHeight: 1.35 }}>登录后自动恢复较新的云端数据；本机变更也会自动保存。</div>
        </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
        <div style={{ fontSize: 12, color: "var(--text-sec)", lineHeight: 1.5 }}>只同步已完成的真实专注块，不会把草稿写进日历。</div>
